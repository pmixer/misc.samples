...
def output_post_save(model, os_path, contents_manager):
    ...
    if ftype == "notebook":
        # call nbconvert
        command1 = "jupyter-nbconvert --to markdown {}".format(
                os_path)
        command2 = "move {}.md {}".format(os.path.join(pjoin(_BLOG_ROOT, 'jupyters'), title), os.path.join(dest_name, dest_dir))
        # input('command1: {} finished'.format(command1))
        subprocess.check_output(command1, shell=True)
        # input('command2: {} finished'.format(command2))
    ...
    if mkdocs.get('nav') is None:
        mkdocs['nav'] = []
    pages = mkdocs.get('nav')
    cexist = list(filter(lambda p: p.get(category), mkdocs.get('nav')))
